    ...
    with open(filePath, "w") as outputWriter:
        ...
        outputWriter.write(
            "  {0}\n".format(
                "  ".join(
                    [
                        "{0: >3}".format(len(atomGroups[symbol]))
...
def ParseOUTCAR(filePath, extractList):
    ...
    with open(filePath, "r") as inputReader:
        ...
        for line in inputReader:
            if "POMASS" in line:
                ...
                if match:
                    ...
                    typeMasses.append(float(match.group("pomass")))
        ...
        for line in inputReader:
                ...
                if line == "BORN EFFECTIVE CHARGES (in e, cummulative output)":
                    ...
                    for _ in range(0, numAtoms):
                        ...
                        for i in range(0, 3):
                            ...
                            becTensor.append([float(i1), float(i2), float(i3)])

                        becTensors.append(np.array(becTensor, dtype=np.float64))
            ...
            if "epsilon_static" in extractList:
                ...
                if (
                    line[:36] == "MACROSCOPIC STATIC DIELECTRIC TENSOR"
                    ...
                    for i in range(0, 3):
                        ...
                        dielectricTensor.append([float(i1), float(i2), float(i3)])
                ...
                if line == "Eigenvectors and eigenvalues of the dynamical matrix":
                    ...
                    for i in range(0, 3 * numAtoms):
                        ...
                        if int(match.group("mode_index")) != i + 1:
                            raise Exception(
                                'Error: Unexpected mode ordering in a frequencies/eigenvectors block in OUTCAR file "{0}".'.format(
                                    filePath
                                )
                            )
                        ...
                        for i, groupName in enumerate(
                            ["freq_thz", "freq_radthz", "freq_invcm", "freq_mev"]
                        ):
                        ...
                        for j in range(0, numAtoms):
                            eigenvector.append(
                                [
                                    float(element)
                                    for element in next(inputReader)
                                    .strip()
                                    .split()[-3:]
                                ]
                            )
    ...
    for tag in extractList:
        if tag not in outputData:
            raise Exception(
                "Error: Failed to extract data for tag '{0}' from OUTCAR file \"{1}\".".format(
                    tag, filePath
                )
            )
